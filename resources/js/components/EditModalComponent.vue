                ...
                <div class="modal-body">
                    <!-- <form v-if="type === 1"> -->
                      <form >
                        ...
                        <div class="form-group">
                            <label for="recipient-name" class="control-label">Cambiar imagen</label>
                            <div class="input-group mb-3">
                              <input type="file" ref="fileInput" class="form-control" @change="processFile($event)" aria-describedby="basic-addon2">
                            </div>
                        </div>
                        <div class="form-group">
                            <center>
                            <button @click.prevent="addWish()" :data-dismiss="checkmodal" type="button" class="btn btn-success">Cambiar</button>
                            </center>
                        </div>
...
export default {
  ...
  created(){
    this.$eventHub.$on('editModal', (row) => {
       this.name = row.name
  ...
  methods : {
      ...
      addWish : function (){
        ...
        .then(r => {
          if(r.data.success) {
            swal('El deseo se actualizo correctamente','','success')
